    ...
    debounce(async (value, setFieldValue, inputs) => {
      ...
      if (value.length == 5 && value.toUpperCase() != 'C99') {
        ...
        if (Object.keys(res.data).length !== 0) {
          ...
          setPaymentCheck(value.toUpperCase().includes('C') ? 0 : null)
          setSearch('')
          setDevedor(res.data.devedor)
          setFieldValue('nome', res.data.nome_cartao)
          setFieldValue('cpf', res.data.cpf)
          ...
          setFieldValue('unidade', res.data.i_unidade || 2000003)
          setFieldValue('local', 
            (parseFloat(params) === 2 && value.toUpperCase().includes('C') ? 0 : null) === 0 ? 4 : parseFloat(params) || 
            (parseFloat(params) === 1 && value.toUpperCase().includes('C') ? 0 : null) === 0 ? 3 : parseFloat(params) || 
            inputs.local
          )
          ...
          setPagador({
            ...
            data: `${year}-${month}-${day}T${hours}:${minutes}`,
            nome: res.data.nome_cartao,
            local: (parseFloat(params) === 2 && value.toUpperCase().includes('C') ? 0 : null) === 0 ? 4 : parseFloat(params) || 
              (parseFloat(params) === 1 && value.toUpperCase().includes('C') ? 0 : null) === 0 ? 3 : parseFloat(params) || 
              inputs.local || 0,
            unidade: res.data.i_unidade || 2000003,
            tipo: res.data.tipo,
  ...
  const inputSearchNameChange = useCallback(
    debounce(async (value, setFieldValue, inputs) => {
      ...
      if (Object.keys(res.data).length !== 0) {
        ...
        setFieldValue('cpf', res.data.cpf)
        setFieldValue('data', `${year}-${month}-${day}T${hours}:${minutes}`)
        if(res.data.i_unidade){
          setFieldValue('unidade', res2.data.filter((i:any)=>i.i_unidade === res.data.i_unidade)[0].i_unidade)
        } else {
          setFieldValue('unidade', 2000003)
        ...
        setFieldValue('local', value.value.cod_biblioteca.toUpperCase().includes('C') ? parseFloat(params) === 1 ? 3 : 4 : parseFloat(params) || inputs.local)
        setCodeSearch(value.value.cod_biblioteca)
        setPaymentCheck(value.value.cod_biblioteca.includes('C') ? 0 : null)
        setDevedor(res.data.devedor)
        setPagador({
          cpf: res.data.cpf,
          data: `${year}-${month}-${day}T${hours}:${minutes}`,
          nome: res.data.nome_cartao,
          local: value.value.cod_biblioteca.toUpperCase().includes('C') ? parseFloat(params) === 1 ? 3 : 4 : parseFloat(params) || inputs.local || 0,
          unidade: res.data.i_unidade || 2000003,
          tipo: res.data.tipo,
          radio:value.value.cod_biblioteca.includes('C') ? 0 : 1
        })
  ...
  return (
    <section>
      ...
      <div className="container-fluid" style={end ? {filter:'blur(3px)'}:{}}>
        <Formik
          initialValues={{ nome: '', cpf: '', local: 0, unidade: '', data: '', radio:0}}
          ...
          onSubmit={(values: pagador, { setSubmitting }) => {
            ...
            setPagador({...values, tipo:pagador?.tipo, local:Number(values.local)})
            console.log(pagador);
            setEnd(true)
            setSubmitting(false)
          }}
        ...
        >
          {({ setFieldValue, errors, touched, values, submitForm }) => (
            <div className="row" style={{height:'100vh'}}>
              ...
              {/* left */}
              <div className="col-sm-12 col-md-6 col-lg-7 col-xl-7" style={{height:'100vh'}}>

                {/* search */}
                <div className="row m-3" style={{height:70}}>
                  <div className="col card border-0 p-0 shadow-sm">
                    <div className="card-body p-0 d-flex align-items-center">
                      <div className="col-sm-1 p-3 ps-4 searchLupa">
                        <i className="bi bi-search h4 text-body"></i>
                      </div>
                      <div className="d-flex h-100 w-100 ms-2">
                        {!searchName ?
                          <input
                            className="form-control-borderless w-100"
                            type="search"
                          ...
                          :
                          <Select
                            className='basic-single w-100 pe-3 h-100'
                            styles={colourStyles}
                            unstyled
                            options={optionsUser}
                            menuPlacement='auto' 
                        ...
                        <div className='d-flex gap-1 align-items-center col-xxl-2 col-xl-3 col-lg-3 col-md-4 col-sm-4'>
              ...
              {/* right */}
              <div className="col-sm-12 col-md-6 col-lg-5 col-xl-5" style={{height:'100vh', overflowY:'auto'}}>
                ...
                <div className="col" >
                  <div className='p-3 m-3 bg-white shadow-sm rounded'>
                    <div className='d-flex justify-content-between'>
                      ...
                      <button type="button" className="btn btn-light py-1" style={{color:'rgb(230, 65, 92)'}} onClick={_ => setServicosCart([])}>
                    ...
                    <div className="col" style={{ maxHeight: '300px', overflowY: 'auto' }}>
                      <ul className="list-group list-group-flush" >
                        {servicosCart.length != 0 ? servicosCart.map((item,k) => (
                          ...
                          :
                          <div>
                            <i className="bi bi-emoji-frown d-flex justify-content-center fs-1 text-secondary"></i>
                ...
                <Form>
                  <div>
                    ...
                    <div className='mx-3 px-3 d-flex justify-content-between'>
                      <h4>TOTAL</h4>
                      {paymentCheck === 0 ?
                        <h4>{formatValue({value: (Number(total) || 0).toString(), groupSeparator: '.', decimalSeparator: ',', decimalScale: 3, prefix: 'R$ '})}</h4>
                        :
                        <h4>{formatValue({value: (Number(total) + devedor || 0).toFixed(2), groupSeparator: '.', decimalSeparator: ',',decimalScale: 2, prefix: 'R$ '})}</h4>
                      }
                    </div>

                    <div className='mx-3 p-3 rounded' style={{backgroundColor:'#a2cff9a6'}}>
                    ...
                    <button 
                      className='mx-3 mb-0 my-3 fs-4 text-light btn-original' 
                      type='submit' 
                      onClick={()=>submitForm()} 
                      disabled={paymentCheck === null || ((Number(total) + devedor) === 0 && paymentCheck !== 0) || !pagador}
                    >
                      FINALIZAR COMPRA
                    </button>
                    <small className='d-flex justify-content-center m-0'>Versão {version}</small>
                  </div>
                </Form>
              </div>
            </div>
