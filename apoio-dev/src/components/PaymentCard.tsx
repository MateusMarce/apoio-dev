...
function PaymentCard({
  ...
  if(codeSearch) code = codeSearch.indexOf('C') !== -1 ? true : false

  useEffect(()=>{
    setChecked(paymentCheck === keyK
    )
  },[codeSearch, keyK, paymentCheck])
  ...
  return (
    <div className="div-btn-input" title={nome} key={keyK} style={{position:'relative', width:'25%'}}>
      ...
      <Field
        ...
        id={nome}
        disabled={
          ((nome == 'Conta' || nome == 'SATC') && pagador?.tipo === 'E' && !colaborador)
          || ((nome == 'Conta' || nome == 'Dinheiro' || nome == 'PIX') && pagador?.tipo === 'I')
          || (nome == 'SATC' && pagador?.tipo === 'A')
        }
