...
function ModalEnd({
  ...
  menuValue
    ...
    let date = pagador.data.split('T')[0]
    let data = new Date()
    let year = date.split('-')[0]
    let month = date.split('-')[1]
    let day = date.split('-')[2]
    let hours = data.toLocaleString("default", { hour: "2-digit" })
    let minutes = data.toLocaleString("pt-br", { minute: "2-digit" })
    var pay: string = 'PIX'
        ...
        case 'PIX':
          ...
          if(!cancel) {
            try {
              ...
              apoio_user = {...apoio_user, vl_recibo:valor, vl_dinheiro: valor, cpf_cnpj:pagador.cpf.replaceAll('.','').replaceAll('-','').replaceAll('/',''), vl_troco:'0.00'}
              console.log(apoio_user);
              
              var res = await axios.post('api/apoio/novo-atendimento', apoio_user)
              handleInterval(res.data.i_apoio)
              window.open(`${import.meta.env.VITE_API_BASE_URL_PIX}/apoio?valor=${valor}&nome=${pagador.nome}&cpf_cnpj=${pagador.cpf.replaceAll('.','').replace('-','').replaceAll('/','')}&origem=${pagador.local}&i_apoio=${res.data.i_apoio}`, '_blank')
    ...
    let local_string;
    switch (pagador.local) {
        case 1:
            local_string = 'Apoio I'
            break
        case 2:
            local_string = 'Apoio II'
            break
        case 3:
            local_string = 'Serviços Internos I'
            break
        case 4:
            local_string = 'Serviços Internos II'
            break
    }
        ...
        <div className="w-100 h-100 d-flex justify-content-center align-items-center" style={{position:'absolute', top:0, zIndex:999, backgroundColor:'rgba(0,0,0,0.2)'}}>
            <div style={{width:500, backgroundColor:'white'}} className='shadow-lg rounded-5 row py-3' >
                <div className="d-flex justify-content-end" style={{height:30}}>
                    <button className="btn btn-white p-0 px-2" onClick={()=>setEnd(false)}><i className="bi bi-x-lg"></i></button>
                </div>
                <div className="px-4">
                    <div className="row mb-2">
                        ...
                        <h5 className="d-flex justify-content-center mt-2" style={{fontSize:18}}>Pagamento {mode === 1 ? 'em': 'por'} {pay}</h5>
                        {pay == 'SATC' ?
                          <h5 className="d-flex justify-content-center" style={{fontSize:25}}>{formatValue({value: total, groupSeparator: '.', decimalSeparator: ',', prefix: 'R$ ', decimalScale: 3})}</h5>
                          :
                          <h5 className="d-flex justify-content-center" style={{fontSize:25}}>{formatValue({value: (devedor > 0 ? Number(total) + devedor : Number(total)).toFixed(2), groupSeparator: '.', decimalSeparator: ',', prefix: 'R$ ', decimalScale: 2})}</h5>
                        }
                    </div>
                    <h5 className='border-top pt-4'>Informações da venda:</h5>
                    ...
                    <div className="d-flex justify-content-between border-bottom mb-4 pb-4">
                        <p className="mb-1">{menuValue.label}</p>
                        <p className="mb-1">{local_string}</p>
                    </div>
                    ...
                    <div className='overflow-scroll pe-3' style={{maxHeight:150, }}>
                ...
                <div>
                  ...
                  {pay == 'dinheiro' && <button
                    ...
                    style={{width:'calc(100% - 1rem)'}}
                    onClick={handleClickFinalizar}
                    disabled={parseFloat(troco.replace(',','.') || '0') < Number(total)|| submitting}
                  >
                    CONCLUIR VENDA
                  </button>}
                </div>
